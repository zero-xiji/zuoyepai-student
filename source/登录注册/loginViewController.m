...
- (IBAction)is_login:(id)sender {
    ...
    else
    {
        _tips.text=@"";
        //1.请求的网址：即请求的接口，
        NSString *urlString = [NSString stringWithFormat:@"http://193.112.2.154:7079/SSHtet/login?table=student&user_name=%@&password=%@",_username.text,_password.text];
        ...
        NSString *str =[[NSString alloc]initWithData:data_student encoding:NSUTF8StringEncoding];
...
-(void)setUser:(NSArray *)array
{
    ...
    this_user_ = [user UserWithName:[array objectAtIndex:3]
               THIS_STUDENT_USER_ID:[array objectAtIndex:5]
         THIS_STUDENT_USER_PASSWORD:[array objectAtIndex:6]
         THIS_STUDENT_USER_TOUXIANG:[array objectAtIndex:2]
