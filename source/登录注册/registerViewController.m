...
- (IBAction)is_register:(id)sender
{
    ...
    else
    {
        _tips.text=@"";
        //1.请求的网址：即请求的接口，
        NSString *urlString = [NSString stringWithFormat:@"http://193.112.2.154:7079/SSHtet/register?table=student&user_name=%@&password=%@&touxiang=%@&school_id=%@",_username.text,_password.text,_now_touxiang,_school.text];
...
//        NSURL *url =[NSURL URLWithString:[NSString stringWithFormat:@"http://193.112.2.154:7079/SSHtet/register?table=student&user_name=%@&password=%@&touxiang=%@&school_id=%@",_username.text,_password.text,_now_touxiang,_school.text]];
